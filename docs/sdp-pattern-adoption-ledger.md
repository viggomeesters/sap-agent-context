...
| Identity / alias / relation contracts | Private SDP schema module table definitions for objects, aliases, relations, field identity | MIT-labelled source repo; pattern description only | false | Make object identity, aliases, relations and field identity explicit, testable contracts. | Planned child `identity-alias-relation-contracts` | Adopt contract shape only; no object rows or aliases copied. |
